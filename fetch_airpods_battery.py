...
def get_battery_from_data(data_hexa):
    ...
    left_status = (100 if status_tmp == 10 else
                   (status_tmp * 10 + 5 if status_tmp <= 10 else ''))

    status_tmp = int("" + chr(data_hexa[13 if flip else 12]), 16)
    right_status = (100 if status_tmp == 10 else
                    (status_tmp * 10 + 5 if status_tmp <= 10 else ''))

    status_tmp = int("" + chr(data_hexa[15]), 16)
    case_status = (100 if status_tmp == 10 else
                   (status_tmp * 10 + 5 if status_tmp <= 10 else ''))

    charging_status = int("" + chr(data_hexa[14]), 16)
    charging_left: bool = (charging_status &
                           (0b00000010 if flip else 0b00000001)) != 0
    charging_right: bool = (charging_status &
                            (0b00000001 if flip else 0b00000010)) != 0
    charging_case: bool = (charging_status & 0b00000100) != 0

    res = "L:" + add_color_zsh_prompt(
        left_status) + ' ' if left_status != '' else ''
    res += "R:" + add_color_zsh_prompt(
        right_status) + ' ' if right_status != '' else ''
    res += "C:" + add_color_zsh_prompt(
